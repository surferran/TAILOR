"""
desire:
    display system points on basic map image.
...
G_wVirtualEdges = nx.MultiDiGraph(name="Full System Graph")
...
#        Gimgs.add_node(unit_img)
        ...
        for var in inVars:
            ...
            G_wVirtualEdges.add_edge(source, unit_name, var_name=name, units=units)
...
G_wVirtualEdges.edges(data=True)
...
def max_pos(pos):
    ...
    for p in pos.keys():
#        print p
        maxX = max(maxX, pos[p][0])
        maxY = max(maxY, pos[p][1])
        minX = min(minX, pos[p][0])
        minY = min(minY, pos[p][1])
    return maxX, maxY, minX, minY
    
maxX, maxY, minX, minY = max_pos(pos)
print maxX
print maxY
print minX, minY
...
circleBuffer = node_circle_size/10
ax.axes.set_xlim(minX-circleBuffer, maxX+circleBuffer)
ax.axes.set_ylim(minY-circleBuffer, maxY+circleBuffer)

tmp = G.adjacency_list()
tmp2 = G.edges(data=True)
tmp3 = nx.to_pandas_dataframe(G)
#G.has_successor('physics')?
G.nodes(0)
G.nodes(1)
G.nodes()
G.in_degree(G.nodes())
...
G.adj
G.edges('physics',data=True)
baseNode = 'payload'
eOut = G.out_edges(baseNode, data=True)
eIn = G.in_edges(baseNode, data=True)
nodeEdges = eOut + eIn
...
if 1==2:
    ax = plt.axes()
    arrowLine = ax.arrow(0, 0, 0.5, 0.5, head_width=0.05, head_length=0.1, fc='k', ec='k')
    plt.show()
''''''
...
def on_press(event):
    ...
    print('button_on_press=%d, x=%d, y=%d, xdata=%f, ydata=%f' %
